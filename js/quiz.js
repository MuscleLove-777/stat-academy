...
const Quiz = {
    ...
    render() {
        ...
        html += `
                <div class="quiz-actions">
                    ...
                    `}
                    <button class="btn btn-outline" onclick="App.showModule(${this.currentModuleId})">学習に戻る</button>
                </div>
            </div>
        `;
        ...
        if (q.type === 'choice') {
            ...
            q.options.forEach((opt, optIdx) => {
                ...
                if (this.submitted) {
                    ...
                    else if (this.answers[q.id] === optIdx && optIdx !== q.answer) optClass += ' incorrect';
            ...
            html += `
                <div class="quiz-fill-blank">
                    <input type="text" data-qid="${q.id}" placeholder="回答を入力..."
                    ...
                    ${this.submitted ? `<div style="margin-top:8px;font-size:0.9rem;">正解: <strong style="color:var(--success)">${q.answer}</strong></div>` : ''}
                </div>
            `;
        }
    ...
    submit() {
        this.currentQuiz.forEach(q => {
            if (q.type === 'fill') {
                ...
                if (input && input.value.trim()) this.answers[q.id] = input.value.trim();
            }
        });

        const unanswered = this.currentQuiz.filter(q => this.answers[q.id] === undefined);
        if (unanswered.length > 0) {
            if (!confirm(`未回答の問題が ${unanswered.length} 問あります。このまま採点しますか？`)) return;
        }
        ...
        modal.innerHTML = `
            ...
            <p>
                ...
                ${result.passed
                    ...
                    : '<br>合格ラインは70%です。解説を確認して再挑戦しましょう。'}
            </p>
            <div class="modal-actions">
                ${result.passed ? `<button class="btn btn-success" onclick="Quiz.completeAndNext()">次のモジュールへ</button>`
    ...
    finish() {
        const result = this.calculateScore();
        if (result.passed) App.goToNextModule(this.currentModuleId);
